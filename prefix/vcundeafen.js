...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!member) return message.reply({ embeds: [new EmbedBuilder().setColor('Red').setDescription("❌ Mention a user to undeafen in VC.")] });

    if (!member.voice.channel) {
      return message.reply({ embeds: [new EmbedBuilder().setColor('Red').setDescription("❌ User is not in a voice channel.")] });
    }

    await member.voice.setDeaf(false, `${message.author.tag}`);
    return message.reply({ embeds: [new EmbedBuilder().setColor('Green').setDescription(`🔊 Undeafened ${member.user.tag} in VC.`)] });
  }
};
