...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!user) {
            return message.reply({
                embeds: [embed.setDescription(`❌ Please mention a user or provide their ID.`)]
            });
        }

        if (!amount || isNaN(amount)) {
            return message.reply({
                embeds: [embed.setDescription(`❌ Please provide a valid amount.`)]
            });
        }

        if (!['s', 'm', 'h', 'd'].includes(timeType)) {
            return message.reply({
                embeds: [embed.setDescription(`❌ Invalid time type. Use one of: s, m, h, d.`)]
            });
        }

        if (!user.moderatable) {
            return message.reply({
                embeds: [embed.setDescription(`❌ I cannot mute this user.`)]
            });
        }
        ...
        if (!muteDuration || muteDuration <= 0 || muteDuration > maxTimeout) {
            return message.reply({
                embeds: [embed.setDescription(`❌ Invalid duration. Must be greater than 0 and not more than 28 days.`)]
            });
        }
        ...
        await user.timeout(muteDuration, reason).catch(err => {
            return message.reply({
                embeds: [embed.setDescription(`❌ Failed to mute user: ${err.message}`)]
            });
        });
        ...
        const replyEmbed = new EmbedBuilder()
            .setColor('Green')
            .setDescription(`✅ <@${user.id}> has been muted for **${amount} ${timeType}**.\n**Reason:** ${reason}`);
